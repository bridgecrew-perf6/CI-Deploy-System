...
layui.define(["table", "form"], function(t) {
	...
	i.render({
		...
		url: "/deploy/projectversion/listdata",
		...
		var id=e.id;
		if ("del" === t.event) {
			layer.confirm("确定删除此版本？", function(e) {
		...
		if ("edit" === t.event) {
			layer.open({
		          type: 2
		          ,title: '编辑版本'
		          ,content: '/deploy/projectversion/edit?id=' + id + '&v='+Math.random()
		          ,maxmin: true
		          ,area: ['600px', '450px']
		          ,btn: ['确定', '取消']
...
//						});
						...
						$.ajax({
				  			type: "POST",
				  			url: "/deploy/projectversion/edit",
				  			data: i.field,
				  			dataType: "json"
				  		}).done(
				  		function(result) {
