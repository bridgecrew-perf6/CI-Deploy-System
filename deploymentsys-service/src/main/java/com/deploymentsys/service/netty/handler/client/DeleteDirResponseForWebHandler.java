...
public class DeleteDirResponseForWebHandler extends SimpleChannelInboundHandler<DeleteDirResponsePacket> {
	private static final Logger LOGGER = LoggerFactory.getLogger(DeleteDirResponseForWebHandler.class);
	...
	public DeleteDirResponseForWebHandler() {
	}

	public DeleteDirResponseForWebHandler(DeployTaskServerToDoBean toDo) {
		this.toDo = toDo;
	}
	...
	public void channelActive(ChannelHandlerContext ctx) {
		...
		try {
			DeleteDirRequestPacket request = new DeleteDirRequestPacket();

			request.setFilePathForDelete(toDo.getParam1());
	...
	@Override
	protected void channelRead0(ChannelHandlerContext ctx, DeleteDirResponsePacket deleteDirResponsePacket) {
		try {
			if (deleteDirResponsePacket.getResponseCode() != 0) {
				logMsg = MessageFormat.format("远程路径删除任务失败 {0}，接收端异常信息：{1}", JSON.toJSONString(toDo),
						deleteDirResponsePacket.getResponseMsg());
				// 更新部署任务状态为失败
				toDo.setStatus(DeployTaskStatus.FAILURE);
			} else {
				boolean done = deleteDirResponsePacket.isDone();

				logMsg = MessageFormat.format("远程路径删除任务{0} {1}, responseMsg: {2}", done ? "成功" : "失败",
						JSON.toJSONString(toDo), deleteDirResponsePacket.getResponseMsg());
