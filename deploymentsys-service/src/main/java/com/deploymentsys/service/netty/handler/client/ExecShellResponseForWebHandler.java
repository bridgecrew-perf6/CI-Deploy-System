...
public class ExecShellResponseForWebHandler extends SimpleChannelInboundHandler<ExecShellResponsePacket> {
	private static final Logger LOGGER = LoggerFactory.getLogger(ExecShellResponseForWebHandler.class);
	...
	public ExecShellResponseForWebHandler() {
	}

	public ExecShellResponseForWebHandler(DeployTaskServerToDoBean toDo) {
		this.toDo = toDo;
	}
	...
	public void channelActive(ChannelHandlerContext ctx) {
		...
		try {
			ExecShellRequestPacket request = new ExecShellRequestPacket();

			request.setShell(toDo.getParam1());
	...
	@Override
	protected void channelRead0(ChannelHandlerContext ctx, ExecShellResponsePacket responsePacket) {
		try {
			if (responsePacket.getResponseCode() != 0) {
				logMsg = MessageFormat.format("远程执行命令任务失败 {0}，接收端异常信息：{1}", JSON.toJSONString(toDo),
						responsePacket.getResponseMsg());
				// 更新部署任务状态为失败
				toDo.setStatus(DeployTaskStatus.FAILURE);
			} else {
				boolean done = responsePacket.isDone();

				logMsg = MessageFormat.format("远程执行命令任务{0} {1}, responseMsg: {2}", done ? "成功" : "失败",
						JSON.toJSONString(toDo), responsePacket.getResponseMsg());
